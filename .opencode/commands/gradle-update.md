...
- If a newer stable Gradle is available, run `./gradlew wrapper --gradle-version=<latest> --distribution-type all`
  **twice** — the first run rewrites `distributionUrl`, the second (under the new version) regenerates the wrapper jar
  and `gradlew`/`gradlew.bat`; without the flag the distribution type flips to `-bin`, and a single run leaves the jar
  and scripts at the old version. Then verify the build still passes (e.g. `./gradlew compileJava`), and report the
  version change. Do not use `release-candidate`/`nightly` labels — only stable versions.
