...
- When the user provides a note (text after `/ideas`), append it as a dated bullet under a `## YYYY-MM-DD` section in
  `docs/ideas.md`, creating the section for today if it does not exist. Use the current date, and do not hand-wrap the
  prose — `docs/ideas.md` is formatter-wrapped. Do not restructure existing entries.
