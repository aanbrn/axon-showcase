...
- **Saga & Deadline Overhaul**: Sagas and Deadlines are not available in Axon Framework 5 — there is no native "async-native" replacement yet, and their future is still being decided.
- **Event Upcasters**: The Axon Framework 4 upcaster SPI is not reintroduced. As of 5.2.0 its successor is message transformation (`@Event(version = ...)` plus the commercial `io.axoniq.framework:axoniq-message-transformation` module).
- **Event Replaying**: Available in the core (open-source) framework — `StreamingEventProcessor.resetTokens()` (with overloads) replays a streaming processor from a chosen position, with `@ResetHandler`, the `ReplayToken` / `@ReplayContext` support, and `EventTrackerStatus.isReplaying()`.
- **Multiple Event Sources**: `MultiStreamableEventSource` is available in the commercial `io.axoniq.framework:axoniq-event-streaming` module (`io.axoniq.framework.messaging.eventstreaming.MultiStreamableEventSource`, implementing `StreamableEventSource`) — on Maven Central since 5.1.0.
- **Annotated Features (delivered in 5.2.0)** — no longer pending; available as of Axon Framework 5.2.0:
    *   **Timeouts**: Built-in timeout support for message handlers and processing contexts.
    *   **Interceptors**: Declarative and annotated interceptor support (`@CommandHandlerInterceptor` / `@EventHandlerInterceptor` / `@QueryHandlerInterceptor`) for message handling components.
