...
As of **Axon Framework 5.2.0**, interceptor methods can be declared directly within handler classes using the per-message-type annotations `@CommandHandlerInterceptor`, `@EventHandlerInterceptor`, and `@QueryHandlerInterceptor`. The method contract differs from Axon Framework 4: an intercepting method takes a `MessageHandlerInterceptorChain` parameter and returns a `MessageStream`. (The `@MessageHandlerInterceptor` annotation as a *method*-level declaration is not the mechanism for this — use the per-type annotations above, or implement the `MessageHandlerInterceptor<M>` interface.)
