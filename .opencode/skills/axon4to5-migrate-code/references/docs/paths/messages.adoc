...
| `@TargetEntityId` + `@Command(routingKey = "…")`
| Axon Framework 5 moves from the "Aggregate" terminology towards a more general "Entity" model. This aligns with the Dynamic Consistency Boundary (DCB) principles where boundaries are more fluid. Note that AF4's `@TargetAggregateIdentifier` also served as the routing key; in AF5 the routing key must be declared explicitly via `@Command(routingKey = "…")` to keep same-target commands handled sequentially. See the xref:migration:understanding-architecture-principles.adoc#moving-away-from-aggregates[architecture principles] for more details.
...
[IMPORTANT]
====
In Axon Framework 4, `@TargetAggregateIdentifier` did two things: it identified the target *and* acted as the command's routing key, guaranteeing that commands for the same aggregate were handled by the same instance, one at a time. `@TargetEntityId` only does the first. To preserve the sequential-handling guarantee you must also make the routing key explicit with `@Command(routingKey = "<field>")` — the field annotated with `@TargetEntityId`. Effectively, AF4's `@TargetAggregateIdentifier` equals AF5's `@TargetEntityId` *plus* `@Command(routingKey = "…")`.

Omitting the `routingKey` compiles and passes unit tests, but under concurrent load commands with the same target ID may be handled in parallel, breaking state-based validation.
====
