...
- **The guidance and tooling are audited, not just appended to.** `/audit-agents` runs an on-demand, pro-model audit of
  ...
  (duplication, trivia, length, placement) — each with a concrete suggested rewrite, including standing merge and
  removal candidates — and, separately and without severity, a third-party file our usage contradicts and the meta rules
  that accreted with the origin that introduced each — decisions for the owner rather than defects to fix. So the
...
OpenSpec config's declared list surfaces (its artifact rules and its operation guidance). The Gradle cache is restored
