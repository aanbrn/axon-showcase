...
`application.yml` `${ENV:default}` placeholders and the Helm chart values mirror it, using the same default as the Java
field, so a deployment with no env overrides behaves exactly as the Java contract declares. A property that must differ
in a given deployment is overridden through env vars, not by changing the default in the yml or chart.
