...
correction deferred, because narrowing it breaks `:showcase-query-client:compileJava` (that consumer receives a
transitive `hibernate-validator` through it) — with no artifact recording whether the direction is allowed.
...
landed, where naming the forbidden edges is stable and derivable from the documents. `showcase-query-api` →
`showcase-command-api` is a contract-to-contract edge, outside the forbidden classes, and is **sanctioned**: it stands
on the transitive `hibernate-validator` its consumers need, recorded in `AGENTS.md`. Narrowing it, and having those
consumers declare what they actually use, remains a parked idea.
...
- Contract-to-contract edges are unconstrained, so a dependency between the `-api` modules would pass unnoticed; a
  narrower rule has this ADR's stated shape to grow from — as does the parked `query-api` narrowing.
