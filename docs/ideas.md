...
- Web-based UI — build a beautiful web-based UI for the showcase so the CQRS/Event-Sourcing pipeline can be interacted
  with and demonstrated visually rather than only via the REST API / `curl`. Parked; no change yet. Open questions for
  later: single-page app served by the API gateway, what it interacts with (create/browse showcases, event timeline),
  and how it fits the existing read/write-side separation.
