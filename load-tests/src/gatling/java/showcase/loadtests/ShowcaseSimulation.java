...
public class ShowcaseSimulation extends Simulation {
    ...
    private static final HttpRequestActionBuilder scheduleShowcase =
            http("ScheduleShowcase").post("/showcases").asJson().body(StringBody("""
                                             {
                                                "title": "#{title}",
                                                "startTime": "#{startTime}",
                                                "duration": "#{duration}"
    ...
    private static final HttpRequestActionBuilder fetchShowcases =
            http("FetchShowcases").get("/showcases");
    ...
    private static final ScenarioBuilder scenario = scenario("Showcase")
            .exitBlockOnFail()
            .on(
                    fetchShowcases,
                    randomSwitch()
                            .on(
                                    percent(50)
                                            .then(
                                                    ...
                                                    scheduleShowcase.check(
                                                            status().is(201),
                                                            jsonPath("$.showcaseId")
                                                                    .saveAs("showcaseId")),
                                                    randomSwitchOrElse()
                                                            .on(percent(95).then(exitHere()))
                                                            .orElse(
                                                                    doWhileDuring(
                                                                                    "#{responseStatus} != 200",
...

    {
        ...
        val injectionProfile =
                switch (testType) {
                    ...
                    case "breakpoint" ->
                        scenario.injectOpen(rampUsersPerSec(0).to(40000).during(Duration.ofHours(2)));
        ...
        val protocol = http.baseUrl(baseUrl).header("Host", "axon-showcase").shareConnections();

        setUp(injectionProfile).assertions(assertions).protocols(protocol);
    }
}
