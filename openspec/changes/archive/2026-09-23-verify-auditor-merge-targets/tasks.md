...
- [x] 1.1 In `.opencode/agent/agents-auditor.md`, state the merge-candidate target check in the conciseness axis and in
      ...
      that reason.
- [x] 1.2 Verify the widened definition keeps every existing element unchanged: the two axes, the merge/removal
...
- [x] 1.3 Verify the definition's wording matches the delta spec's new clause and scenario one-for-one (the target's
...
- [x] 2.1 Re-read the `AGENTS.md` agents-auditor bullet and confirm it already delegates the finding classes to the
      `agent-skills` spec, so this widening owes no bullet edit; if it names the merge-candidate contract inline, update
      it.
- [x] 2.2 Confirm the caller-side rule is already present and consistent with this change — the pointer-verification
...
- [x] 2.3 Confirm the definition's frontmatter `description`, its own report-contract verdict line, the shared
...
- [x] 2.4 Confirm `README.md`'s auditor prose and slash-command table row need no edit — the verification rigor is not
      human-visible behavior — and state the omission in the change's report.
- [x] 2.5 Grep the corpus and docs for the merge-candidate contract's other mentions (`grep -rn "merge candidate"`) and
...
- [x] 3.1 Run `openspec validate --changes` and confirm the change validates with the `MODIFIED` block carrying every
      scenario the main spec's requirement has, plus the new one.
- [x] 3.2 Run `./gradlew spotlessApply` after the final edit to any Spotless-owned file, then confirm `spotlessCheck`
      and the PR gate (`./gradlew check -PskipITs -Pcoverage.gate.enabled=false`) are green.
- [x] 3.3 Smoke-test the widened subagent after an OpenCode reload (a changed definition is read at session start), with
      ...
      the repository before reading the run, and record which branch the run exercised. Run 2026-09-23: `WIDENED: yes`
      (the definition carried the clause verbatim at lines 66-68 and in the conciseness axis); positive control flagged
      — the `deployment/web-ui` target holds the fail-fast behavior and the `config.js` render but not the `start.sh` or
      `/workspace/config.js` identifiers, which the sentence already keeps; negative control not flagged — the
      `gateway/rest-api` target genuinely covers the cache-fallback behavior, so no missing delegated item. Verdict:
      nothing to report beyond the controls.
- [x] 3.4 Run `review-quick` over the implementation diff and fix its findings, re-running until clean; then request the
