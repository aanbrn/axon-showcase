...
- [ ] 1.1 Quote the two `openspec/config.yaml` rule items whose unquoted `: ` makes YAML parse them as mappings
      ...
      it)
- [ ] 1.2 Verify all four artifacts' rules load (`proposal`, `specs`, `design`, `tasks`), with `specs`/`design`
      unchanged as the control, and that `openspec validate --all` is still clean

## 2. Guard it

- [ ] 2.1 Extend the CI `build` job's OpenSpec step in `.github/workflows/ci.yml` with a probe that creates a change,
      ...
      it
- [ ] 2.2 Add the same check to `.opencode/commands/opsx-tool-update.md`, **with a positive control** (temporarily
...
- [ ] 3.1 Add to `AGENTS.md` the YAML pitfall (an unquoted scalar containing `: ` parses as a mapping, so a declared
...
- [ ] 3.2 Audit the `openspec/config.yaml` `context:` block against the project facts it duplicates, correcting the
      ...
      `showcase-web-ui`)
- [ ] 3.3 Refresh the CI-gate enumerations the new step makes stale — `AGENTS.md`'s "Continuous Integration" bullets and
...
- [ ] 4.1 Run `./gradlew check -PskipITs -Pcoverage.gate.enabled=false` (which lints the edited workflow via actionlint)
