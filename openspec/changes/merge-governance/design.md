...
**Goals:**
- A PR gate that runs the Docker-free quality tiers (`check -PskipITs`) plus `openspec validate`, fast enough to be a
...
- Recreating the existing rulesets — they already match the documented baseline; the change records behavior, not
  creation.
- Self-hosted runners, caching farms, or build-cache tuning.
...
### D4: `openspec validate` as an always-on spec gate
