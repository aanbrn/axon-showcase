...
- Add a `ci.yml` workflow that runs on every pull request and on every push to `main`:
  - **Pull request**: fast gate — `./gradlew check -PskipITs` (Docker-free: unit + component + all static gates) plus
    `openspec validate` — the check that the PR ruleset will require
  - **Push to `main`**: full gate — `./gradlew check` (with `integrationTest` via Testcontainers and the JaCoCo
    coverage gate) plus `openspec validate`
