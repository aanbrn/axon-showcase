...
tracked set excludes ignored files, that no tracked file carries a merge conflict marker, and that tracked scripts carry
the executable bit — so slips that were caught only by a human or a review pass are blocked by construction.
