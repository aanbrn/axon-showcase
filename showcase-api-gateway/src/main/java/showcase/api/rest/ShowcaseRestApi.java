...
interface ShowcaseRestApi {
    ...
    @Operation(
            ...
            method = "POST",
            parameters = @Parameter(name = IDEMPOTENCY_KEY_HEADER, description = """
                            ...
                            (e.g., on timeout), so the client can safely retry later.""", in = ParameterIn.HEADER),
            requestBody =
                    @RequestBody(
            ...
            responses = {
                ...
                @ApiResponse(
                        ...
                        content = @Content(mediaType = APPLICATION_PROBLEM_JSON_VALUE, examples = @ExampleObject("""
                                            {
                                              "type": "about:blank",
                                              "title": "Conflict",
                                              "status": 409,
    ...
    Mono<ResponseEntity<ScheduleShowcaseResponse>> schedule(
            @KSUID String idempotencyKey, @Valid ScheduleShowcaseRequest request);
    ...
    @Operation(
            ...
            parameters = {
                ...
                @Parameter(name = "status", description = "The status(es) to filter by.", in = ParameterIn.QUERY),
                @Parameter(
                        name = "afterId",
                        description = "The ID of a showcase after which to fetch.",
                        in = ParameterIn.QUERY),
                @Parameter(name = "size", description = "The number of showcases to fetch.", in = ParameterIn.QUERY)
            },
            responses =
                    @ApiResponse(
                            responseCode = "200",
