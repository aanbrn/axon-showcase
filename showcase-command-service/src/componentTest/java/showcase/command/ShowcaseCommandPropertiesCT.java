...
class ShowcaseCommandPropertiesCT {
    ...
    void applicationYmlPlaceholdersBindDocumentedDefaults() {
        ymlContextRunner.run(context -> {
            ...
            assertThat(properties.getSagaAssociationsCache().getExpiresAfterAccess()).isEqualTo(Duration.ofMinutes(10));
            assertThat(properties.getSagaAssociationsCache().getExpiresAfterWrite()).isEqualTo(Duration.ofMinutes(5));
            assertThat(properties.getShowcaseSnapshotTrigger().getLoadTimeThreshold()).isEqualTo(
    ...
    void envVarFormPropertyOverridesDefault(Map<String, Object> envVars,
                                            Consumer<ShowcaseCommandProperties> assertions) {
        ymlContextRunner
    ...
    static List<Arguments> envVarBindings() {
        return List.of(
                argumentSet("EXIT_AFTER_FLYWAY_MIGRATION",
                            Map.of("EXIT_AFTER_FLYWAY_MIGRATION", "true"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.isExitAfterFlywayMigration()).isTrue();
                            }),
                argumentSet("SHOWCASE_COMMAND_VALIDATION_ENABLED",
                            Map.of("SHOWCASE_COMMAND_VALIDATION_ENABLED", "false"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.isValidationEnabled()).isFalse();
                            }),
                argumentSet("SHOWCASE_CACHE_MAX_SIZE",
                            Map.of("SHOWCASE_CACHE_MAX_SIZE", "2000"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.getShowcaseCache().getMaximumSize()).isEqualTo(2000);
                            }),
                argumentSet("SHOWCASE_CACHE_EXPIRES_AFTER_ACCESS",
                ...
                argumentSet("SHOWCASE_CACHE_EXPIRES_AFTER_WRITE",
                            Map.of("SHOWCASE_CACHE_EXPIRES_AFTER_WRITE", "PT15M"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.getShowcaseCache().getExpiresAfterWrite())
                                        .isEqualTo(Duration.ofMinutes(15));
                            }),
                argumentSet("SAGA_CACHE_MAX_SIZE",
                            Map.of("SAGA_CACHE_MAX_SIZE", "2000"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.getSagaCache().getMaximumSize()).isEqualTo(2000);
                            }),
                argumentSet("SAGA_CACHE_EXPIRES_AFTER_ACCESS",
                            Map.of("SAGA_CACHE_EXPIRES_AFTER_ACCESS", "PT20M"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.getSagaCache().getExpiresAfterAccess())
                                        .isEqualTo(Duration.ofMinutes(20));
                            }),
                argumentSet("SAGA_CACHE_EXPIRES_AFTER_WRITE",
                            Map.of("SAGA_CACHE_EXPIRES_AFTER_WRITE", "PT15M"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.getSagaCache().getExpiresAfterWrite())
                                        .isEqualTo(Duration.ofMinutes(15));
                            }),
                argumentSet("SAGA_ASSOCIATIONS_CACHE_MAX_SIZE",
                            Map.of("SAGA_ASSOCIATIONS_CACHE_MAX_SIZE", "2000"),
                            (Consumer<ShowcaseCommandProperties>) properties -> {
                                assertThat(properties.getSagaAssociationsCache().getMaximumSize()).isEqualTo(2000);
                            }),
                argumentSet("SAGA_ASSOCIATIONS_CACHE_EXPIRES_AFTER_ACCESS",
                ...
                argumentSet("SAGA_ASSOCIATIONS_CACHE_EXPIRES_AFTER_WRITE",
                ...
                argumentSet("SHOWCASE_SNAPSHOT_TRIGGER_LOAD_TIME_THRESHOLD",
