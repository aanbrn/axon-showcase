...
class ShowcaseQueryClientPropertiesCT {
        ...
        contextRunner
                .withInitializer(context -> context.getEnvironment().getPropertySources().addFirst(
                        new SystemEnvironmentPropertySource("test-env-vars",
                                                            Map.of("SHOWCASE_QUERY_API_URL", "http://query:8080"))))
                .run(context -> assertThat(context.getBean(ShowcaseQueryClientProperties.class).getApiUrl())
    ...
    void invalidApiUrlFailsContext(String envValue) {
        contextRunner
                .withInitializer(context -> context.getEnvironment().getPropertySources().addFirst(
                        new SystemEnvironmentPropertySource("test-env-vars",
                                                            Map.of("SHOWCASE_QUERY_API_URL", envValue))))
                .run(context -> {
    ...
    static List<Arguments> invalidApiUrls() {
        return List.of(
                argumentSet("An empty URL", ""),
                argumentSet("A non-URL value", "not-a-url")
        );
    }
